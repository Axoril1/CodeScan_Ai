  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      <div className="dragon-border" style={{
        ...
        <div style={{ paddingLeft: '1rem', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <div key={scan._id} className="dragon-border" style={{ background: '#0d0500', marginBottom: '1rem', overflow: 'hidden' }}>
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              ...
              {expanded === scan._id
                ? <FiChevronUp size={16} color="#c8860a" />
                : <FiChevronDown size={16} color="#c8860a" />
              }
            </div>
          </div>
            ...
            <div style={{ padding: '1.25rem' }} className="fade-in">
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                {scan.result.bugs.length === 0 ? (
                  <div style={{ borderLeft: '3px solid #00a650', padding: '8px 12px', background: 'rgba(0,166,80,0.08)', color: '#50d080', fontSize: '0.85rem', fontWeight: 700 }}>
                    No bugs found.
                  </div>
                ) : scan.result.bugs.map((bug, i) => (
                  <div key={i} style={{ borderLeft: '3px solid #c8380a', padding: '8px 12px', marginBottom: '0.4rem', background: 'rgba(200,56,10,0.08)', fontSize: '0.83rem', fontFamily: "'JetBrains Mono', monospace", color: '#ff9070', fontWeight: 600 }}>
                    {bug}
                  </div>
                ))}
              </div>
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                {scan.result.suggestions.map((s, i) => (
                  <div key={i} style={{ borderLeft: '3px solid #00a650', padding: '8px 12px', marginBottom: '0.4rem', background: 'rgba(0,166,80,0.08)', fontSize: '0.83rem', fontFamily: "'JetBrains Mono', monospace", color: '#50d080', fontWeight: 600 }}>
                    {s}
                  </div>
                ))}
              </div>
